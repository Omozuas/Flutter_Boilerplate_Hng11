...
class _CartHomeScreenState extends State<CartHomeScreen> {
  ...
  getPrice() {
    totalPrice = products.fold(0, (sum, item) => sum + ((item.quantity ?? 0)*(item.price ??0)));
    allPrice = products.fold(0, (sum, item) => sum + ((item.quantity ?? 0)*(item.price ??0)));
    discountedPrice = promoCodeController.text.trim().isEmpty? 0: (totalPrice * (5/100));
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      CustomButton(
                        borderColor: Colors.transparent,
                        text: "Checkout \$1${formatNumber(payPrice, decimalPlaces: 2)}",
                        height: 48.h,
